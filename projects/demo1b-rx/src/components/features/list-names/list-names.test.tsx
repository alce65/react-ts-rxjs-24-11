...
import { of } from 'rxjs';
import { ListNamesIntra, ListNamesSemiIntra } from './list-names-intra';
...
describe('ListNamesIntra component', () => {
    // La lista es fija en el componentes
    // por lo que se testa con los valores esperados

    test('renders names from observable', () => {
        // Test implementation
        render(<ListNamesIntra inputNames={mockNames} />);
        const listItems = screen.getAllByRole('listitem');
        expect(listItems).toHaveLength(4);
        expect(listItems[0]).toHaveTextContent('Pepe');
        expect(listItems[1]).toHaveTextContent('Rosa');
...
describe('ListNamesSemiIntra component', () => {
    // La lista es fija en el componentes
    // por lo que se testa con los valores esperados

    test('renders names from observable', () => {
        // Test implementation
        render(<ListNamesSemiIntra inputNames={mockNames} />);
        const listItems = screen.getAllByRole('listitem');
        expect(listItems).toHaveLength(4);
        expect(listItems[0]).toHaveTextContent('Pepe');
        expect(listItems[1]).toHaveTextContent('Rosa');
